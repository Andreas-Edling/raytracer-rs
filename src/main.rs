...
impl Canvas {
    fn new(width: usize, height: usize) -> Result<Self, String> {
        ...
        unsafe {
            ...
            gl::TexImage2D(gl::TEXTURE_2D, 0, gl::RGBA8 as i32, width as i32, height as i32, 0, gl::RGBA, gl::UNSIGNED_BYTE, canvas.texture_data.as_ptr() as *const std::ffi::c_void);

            let texloc = gl::GetUniformLocation(program, CString::new("tex_sampler").unwrap().as_ptr() as *const i8);
            println!("texloc: {}",texloc);
        ...
        unsafe {
            ...
            gl::BufferData(gl::ARRAY_BUFFER, (std::mem::size_of::<f32>()*canvas.vertices.len()) as isize, canvas.vertices.as_ptr() as *const std::ffi::c_void, gl::STATIC_DRAW);
            let position_location = gl::GetAttribLocation(canvas.program, CString::new("Position").unwrap().as_ptr() as *const i8) as GLuint;
            gl::VertexAttribPointer(position_location, 4, gl::FLOAT, gl::FALSE, 0, std::ptr::null());
...
impl SoftCanvas {
    fn new(width: usize, height: usize) -> Result<Self, String> {
        ...
        let (mut window, events) = glfw.create_window(width as u32, height as u32, "raytracer-rs", glfw::WindowMode::Windowed)
        ...
        Ok(SoftCanvas{canvas, glfw, window, events})
    }
