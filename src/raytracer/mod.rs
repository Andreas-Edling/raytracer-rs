...
fn compute_radiance(scene: &Scene, ray: &Ray, hit: &Hit, sample_generator: &mut SampleGenerator, recursions: u8, spread: u32) -> RGB {
    ...
    let sub_radiance = (0..num_sub_rays).map(|_|{
        let sub_ray = randomize_reflection_ray(sample_generator, hit, ray, &normal);

        let sub_hit = intersect_ray(scene, &sub_ray);

        match sub_hit {
            Some(sub_hit) => compute_radiance(scene, &sub_ray, &sub_hit, sample_generator, recursions-1, spread),
            None => RGB::black(),
        }
    }).fold(RGB::black(), |sum, x| sum + x) * (1.0f32 / num_sub_rays as f32);
